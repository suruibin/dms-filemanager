...
void main() {
    ...
    float band = 0.04;
    float row = floor(uv.y / band);
    float r = fract(sin(row * 127.1 + 311.7) * 43758.5);
    float shift = r * 0.2 * sin(ubuf.progress * 3.14);
    vec4 c = color1;
    if (step(r, ubuf.progress) > 0.5) {
        c = texture(source2, vec2(clamp(uv.x + shift, 0.0, 1.0), uv.y));
    }
