...
void main() {
    ...
    float wave = sin(uv.y * 10.0 - ubuf.progress * 15.0) * 0.5;
    float threshold = smoothstep(ubuf.progress - 0.1, ubuf.progress + 0.1, uv.x + wave);
    fragColor = mix(color1, color2, threshold) * ubuf.qt_Opacity;
}
