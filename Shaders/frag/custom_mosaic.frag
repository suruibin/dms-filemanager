...
void main() {
    ...
    vec2 tile = floor(uv * vec2(8.0, 6.0));
    float r = fract(sin(dot(tile, vec2(12.99, 78.23))) * 43758.5);
    float flip = smoothstep(r - 0.05, r + 0.05, ubuf.progress);
    fragColor = mix(color1, color2, flip) * ubuf.qt_Opacity;
}
